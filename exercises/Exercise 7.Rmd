...
```{r}
#Histogram of continuous variable: hist(data$contvar)
hist(NHANESdata$Age)
NHANESdata%>%
  drop_na(Age)%>%
  group_by(Gender)%>%
...
#3a. What are the means, standard deviations, and counts for age by gender?
NHANESdata%>%
  drop_na(BMI)%>%
  group_by(Gender)%>%
  summarize(mean(BMI), sd(BMI), count=n())
#refutes the alternative hypothesis
...
People without health ins have a statistically higher BMI than people with ins
...
#There is a statistical difference between caloric intake by gender. 
```
...
In the case of the underweight category the alt hypothesis is correct and has a much lower caloric intake than the other categories which don't differ from each other at all.
```{r}
...
summarize(mean(RFITP), sd(HHEducation), count=n())
...
The more education you have, the higher your chances of keeping your family out of poverty.
...
NHANESdata%>%
  drop_na(DR1TCARB)%>%
  group_by(HHFoodSec)%>%
  summarize(mean(DR1TCARB), sd(HHFoodSec), count=n())
dunn.test(x=NHANESdata$DR1TCARB, g=NHANESdata$HHFoodSec, kw=TRUE)
```
